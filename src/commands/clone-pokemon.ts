...
export function clonePokemon(fromPath: string, toPath: string, options: Record<string, string>){
    const originParser = getParser(fromPath);
      ...
      let pokemon;
      if (options.party) pokemon = originParser.readParty()[parseInt(options.party)];
      else {
        const [boxID, position]: string[] = (options.box || "0,0").split(",");
        pokemon = originParser.readBox(parseInt(boxID || "0"))[parseInt(position || "0")];
      }

      destinationParser.writePokemonToBoxSlot(
        pokemon as Pokemon,
        destinationParser.getEmptyBoxSlot(),
        options.out || toPath,
      );
}
